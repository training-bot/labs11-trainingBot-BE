...
router.put("/:id", async (req, res) => {
	...
	try {
		...
		await Notifications.updateNotificationMember(id, updatedNotificationMemberInfo);
	...
	try {
		...
		incomingAssignments.forEach(async (assignment) => {
			try {
				...
				const posts = await TrainingSeries.getTrainingSeriesPosts(newObject.trainingSeries_ID);
				...
				const formattedPosts = posts.map((post) => {
					if (member.emailOn && !member.textOn) {
						return {
							...
							sendDate: moment(startDate).add(post.daysFromStart, "days").format(),
				...
				res.status(500).json({ message: "There was an error assembling the information." });
			}
		});
