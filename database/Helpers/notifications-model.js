...
function getNotificationsToRecalculate(postID, teamMemberID) {
	return db("Notifications")
		...
		.select("Notifications.*", "RelationalTable.startDate")
		.where({ postID: postID });
}

function updateNotificationContent(id, content) {
	return db("Notifications").where({ notificationID: id }).update(content);
}

function updateNotificationMember(id, memberInformation) {
	return db("Notifications").where({ teamMemberID: id }).update(memberInformation);
}
...
async function getTrainingSeriesOfNewPost(id) {
...
function getUserNotificationCountData(id) {
	return db("User")
		.select("User.notificationCount", "accountType.maxNotificationCount", "User.userID")
...
function resetNotificationCount() {
	return db("User").where("notificationCount", ">", 0).update({ notificationCount: 0 });
}

function deleteOldNotifications(today) {
	return db("Notifications").where("sendDate", "<", today).del();
}
